...
// UpsertThumbnail stores or replaces a thumbnail for a device+channel.
func (s *Store) UpsertThumbnail(t *Thumbnail) error {
	_, err := s.DB.Exec(`INSERT INTO thumbnails (device_id, source_id, image_data, timestamp, image_type, image_size_kb)
		...
		t.DeviceID, t.ChannelID, t.ImageData, t.Timestamp, t.ImageType, t.ImageSizeKB,
	)
	return err
}

// GetThumbnail retrieves a thumbnail by device and channel.
func (s *Store) GetThumbnail(deviceID, channelID string) (*Thumbnail, error) {
	t := &Thumbnail{}
	err := s.DB.QueryRow(
		"SELECT device_id, source_id, image_data, timestamp, image_type, image_size_kb FROM thumbnails WHERE device_id = ? AND source_id = ?",
		deviceID, channelID,
	).Scan(&t.DeviceID, &t.ChannelID, &t.ImageData, &t.Timestamp, &t.ImageType, &t.ImageSizeKB)
